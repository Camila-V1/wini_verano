...
String pilaM::InfijaToPostfija(const String& Infija) {
	...
	for (int i = 1; i <= Infija.Length(); i++) {
		...
		if (isdigit(ch)) {
			...
			while (!vacio() && PrioridadInfija(ch) <= PrioridadPila((char)cima())) {
				dir aux;
				Sacar(aux);
				Postfija += (char)aux;
			}
...
String pilaM::ValidarSintaxis(const String& expr, TColor FormColor, TCanvas* Canvas) {
    ...
    if (MessageBox(NULL, L"Validando sintaxis. �Desea continuar?", L"Validaci�n", MB_OKCANCEL) != IDOK) {
...
String pilaM::InfijaToPostfija2(const String& infija, TColor FormColor, TCanvas* Canvas) {
    ...
    if (MessageBox(NULL, L"Convirtiendo infija a postfija. �Desea continuar?", L"Conversi�n", MB_OKCANCEL) != IDOK) {
        return ""; // Cancelado por usuario
    }
...
float pilaM::EvaluarPostfija2(const String& expPostfija, TColor FormColor, TCanvas* Canvas) {
	...
	if (MessageBox(NULL, L"Evaluando expresi�n postfija. �Desea continuar?", L"Evaluaci�n", MB_OKCANCEL) != IDOK) {
		return 0;
	}
