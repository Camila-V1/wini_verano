...
// EN UP_listasP.cpp (Recomendado)
void pilal_P::Sacar(int &e){
    if (!vacio()) { // A�adir esta verificaci�n
       Nodopi* temp = ptrpi;
        e = temp->dato;
        ptrpi = temp->sig;
        delete temp;
        ...
        // Opcional: Manejar el error, como lanzar una excepci�n o mostrar mensaje
        ShowMessage("Error: Intento de sacar de una pila vac�a (pilal_P).");
        // Podr�as asignar un valor por defecto a 'e' si es necesario
        // e = valor_por_defecto;
    }
}
int pilal_P::cima() {
