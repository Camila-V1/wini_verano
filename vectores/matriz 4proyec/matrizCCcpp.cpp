...
int MatrizCC::Existe_Elemento(int f, int c) {
    ...
    for (int z = lug_antes+1; z < VF[f + 1]; z++) {
        if (VC[z] == c) {
            // Si encontramos la columna ocupada, devolvemos la posici�n
            return z;
        }
    }
...
void MatrizCC::eliminarSiIgual(int valor) {
	...
	for (int i = 1; i <= nt; i++) {
		if (VD[i] == valor) {
            ...
            // Continuamos buscando m�s elementos que coincidan
            i--;
        }
    }
}
...
void MatrizCC::imprimir2(TColor FormColor, TCanvas *Canvas, int offsetX, int offsetY) {
	int cellWidth = 40;
	int cellHeight = 30;
