...
int MatrizSC::dimension_columna() {
    ...
    if (Lug != -1) {
        ...
        // Si no encontramos una coincidencia y hay espacio, a�adimos el nuevo valor
        VD[nt] = e;
        Vf[nt] = f;
        VC[nt] = c;
        nt++;
...
void MatrizSC::eliminarSiIgual(int valor) {
	int i = 0;
	while (i < nt) {
        if (VD[i] == valor) {
            for (int j = i; j < nt - 1; j++) {
  ...
  if (nt==0) {
   repe = valor;
     ...
     eliminarSiIgual( valor) ; // este algoritmo no considera si este m�todo es llamado en tiempo de ejecuci�n
   repe=valor;	 }
...
void MatrizSC::imprimir2(TColor FormColor, TCanvas *Canvas, int offsetX, int offsetY) {
	int cellWidth = 40;
	int cellHeight = 30;
