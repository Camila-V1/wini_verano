...
void intersectar_conjuntos(ptr_conjunto* A, ptr_conjunto* B, ptr_conjunto* C) {
    // 1. Limpiar el conjunto C por si ten�a basura de antes
    while (!C->vacio()) {
    ...
    while (!A->vacio()) {
        ...
        if (B->pertenece(e)) {
            C->inserta(e); // Si est� en ambos, se a�ade a la intersecci�n (C)
        }
...
void __fastcall TForm1::Button14Click(TObject *Sender)
{
    ...
    // 3. Configurar las dimensiones del c�rculo
    int centroX = 400; // Posici�n X del centro en la pantalla
    int centroY = 300; // Posici�n Y del centro en la pantalla
    int radio = 150;   // Tama�o del c�rculo
	TColor colorCirculo = Form1->Color; // Color de fondo del c�rculo
...
void diferencia_simetrica_para_M(conjuntoM* a, conjuntoM* b, conjuntoM* c)
{
	if (a == NULL || b == NULL || c == NULL) {
		ShowMessage("Error: Uno o m�s conjuntos no han sido creados.");
		return;
	}
while (!c->vacio()) {
		c->suprime(c->muestrea());
...
void __fastcall TForm1::Button16Click(TObject *Sender)
{
    ...
    if (a != NULL && b != NULL && c != NULL) {
        // Llamado a la funci�n que acabamos de crear
...
{
   pila->Meter(StrToInt(Edit8->Text));
}
//---------------------------------------------------------------------------
...
pila->imprimir(Form1->Color,Canvas);
